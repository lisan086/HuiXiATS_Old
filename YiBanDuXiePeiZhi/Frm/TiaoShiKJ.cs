...
namespace YiBanDuXiePeiZhi.Frm
{
    public partial class TiaoShiKJ : UserControl
    {
        ...
        public void SetCanShu(SaoMaModel maModel,PeiZhiLei peiZhiLei)
        {
            ...
            SaoMaModel = maModel;
            List<CunModel> dumodels = maModel.PeiZhiJiCunQi;
            if (dumodels.Count > 0)
            {
                for (int i = 0; i < dumodels.Count; i++)
                {
                    ...
                    {
                        FuZhi(dumodels[i], true);
                    }
                    else
                    {
                        FuZhi(dumodels[i], false);
                    }
                }
            }
        }
        ...
        public void ShuaXin()
        {
            ...
            for (int i = 0; i < this.dataGridView1.Rows.Count; i++)
            {
                if (this.dataGridView1.Rows[i].Tag is CunModel )
                {
                    ...
                    this.dataGridView1.Rows[i].Cells[3].Value = model.JiCunQi.Value;
                }
            }
            for (int i = 0; i < this.dataGridView2.Rows.Count; i++)
            {
                if (this.dataGridView2.Rows[i].Tag is CunModel)
                {
                    ...
                    {
                        this.dataGridView2.Rows[i].Cells[4].Value ="执行中";
                    }
                    else
                    {
                        this.dataGridView2.Rows[i].Cells[4].Value = model.JiCunQi.Value;
                    }
                }
            }
        }
        ...
        private void FuZhi(CunModel model,bool isdu)
        {
            if (isdu)
            {
                ...
                this.dataGridView1.Rows[index].Cells[2].Value = model.ZhiLing;
            ...
            else
            {
                ...
                this.dataGridView2.Rows[index].Cells[2].Value = model.ZhiLing;
                this.dataGridView2.Rows[index].Cells[3].Value = "";
                this.dataGridView2.Rows[index].Cells[4].Value = "";
                this.dataGridView2.Rows[index].Cells[5].Value = "执行";
            ...
            if (e.RowIndex>=0)
            {
                if (e.ColumnIndex==5)
                {
                    if (this.dataGridView2.Rows[e.RowIndex].Tag is CunModel)
                    {
                        ...
                        xinmodel.JiCunQi.Value = this.dataGridView2.Rows[e.RowIndex].Cells[3].Value;
