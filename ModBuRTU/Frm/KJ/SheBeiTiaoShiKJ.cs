...
namespace SundyChengZhong.Frm.KJ
{
    public partial class SheBeiTiaoShiKJ : UserControl
    {
        ...
        public void SetCanShu(SheBeiModel shebeimodel, PeiZhiLei peiZhi)
        {
            ...
            for (int i = 0; i < SheBeiModel.LisJiCunQis.Count; i++)
            {
                CZJiCunQiModel cunModel = SheBeiModel.LisJiCunQis[i];
                if (cunModel.DataSType == DataSType.DuPuTong)
                {
                    ZengJiaShuJu(cunModel, 1);
                }
                else if (cunModel.DataSType == DataSType.XiePuTong)
                {
                    ZengJiaShuJu(cunModel, 2);
                }
                else if (cunModel.DataSType == DataSType.DuXieYiQi)
                {
                    ZengJiaShuJu(cunModel, 1);
                    ZengJiaShuJu(cunModel, 2);
                }
        ...
        public void ShuaXin()
        {
            ...
            {
                if (this.dataGridView1.Rows[i].Tag is CZJiCunQiModel)
                {
                    CZJiCunQiModel model = this.dataGridView1.Rows[i].Tag as CZJiCunQiModel;
        ...
        private void ZengJiaShuJu(CZJiCunQiModel cunModel, int leixing)
        {
            if (leixing == 1)
            {
        ...
        private void dataGridView2_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                if (e.ColumnIndex == 3)
                {
                    if (this.dataGridView2.Rows[e.RowIndex].Tag is CZJiCunQiModel)
                    {
                        CZJiCunQiModel model = this.dataGridView2.Rows[e.RowIndex].Tag as CZJiCunQiModel;
                        ...
                        PeiZhiLei.ChuFaShiJian(jicunmodel);
                    }
                }
            }
        }
