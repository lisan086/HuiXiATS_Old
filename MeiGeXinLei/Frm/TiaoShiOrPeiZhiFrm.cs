    ...
    public partial class TiaoShiOrPeiZhiFrm : BaseFuFrom
    {
        ...
        private void SetCanShu()
        {
            ...
            else
            {
                ...
                {
                    List<CunModel> lis = PeiZhiLei.DataMoXing.JiLu.Values.ToList();
                    TiaoShiKJ kjs = new TiaoShiKJ();
                    kjs.SetCanShu(lis, PeiZhiLei,this);
                    kjs.Dock = DockStyle.Fill;
                    liskj.Add(kjs);
                    this.panel2.Controls.AddRange(liskj.ToArray());
