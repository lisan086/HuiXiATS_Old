...
namespace YiBanSaoMaQi.Frm
{
    ...
    public class DataMoXing
    {
        ...
        public Dictionary<int, List<byte[]>> TdDuShuJu = new Dictionary<int, List<byte[]>>();
        ...
        public void SetHeGe(int zongid,bool ishege)
        {
            ...
            for (int i = 0; i < LisSheBei.Count; i++)
            {
                if (LisSheBei[i].SheBeiID == zongid)
                {
                    ...
                    {
                        if (JiLu[KeyS[c]].ZongSheBeiId == LisSheBei[i].SheBeiID)
                        {
                             JiLu[KeyS[c]].JiCunQi.IsKeKao=ishege;
                        }
                    }
                    break;
                }
        ...
        public void SetJiCunQiValue(CunModel modelks, object shuju)
        {
            if (JiLu.ContainsKey(modelks.JiCunQi.WeiYiBiaoShi))
            {
                CunModel cunModel = JiLu[modelks.JiCunQi.WeiYiBiaoShi];
                JiLu[modelks.JiCunQi.WeiYiBiaoShi].JiCunQi.Value = shuju;
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        public CunModel IsChengGong(SaoMaModel model,CunType cunType)
        {
            for (int i = 0; i < KeyS.Count; i++)
            {
                if (JiLu[KeyS[i]].ZongSheBeiId == model.SheBeiID && JiLu[KeyS[i]].IsDu == cunType)
                {
                    return JiLu[KeyS[i]];
                }
            }
