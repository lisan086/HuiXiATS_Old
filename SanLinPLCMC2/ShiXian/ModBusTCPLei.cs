...
namespace ModBuTCP.ShiXian
{
    public class ModBusTCPLei : ABSNSheBei
    {
            ...
            while (ZongKaiGuan)
            {
                ...
                try
                {
                    if (lianjieqi.TongXinState)
                    {
                        ...
                        {
                            FuZhi(zsmodel, lis[i], dingJieXi, lianjieqi);
                        }
                    }
        ...
        private void FuZhi(SheBeiModel shebeimodel, JiLuModel model, SerialDataGuDingJieXi jiexieqi, ABSLianJieQi lianjieqi)
        {
            if (model.ShuJu != null)
            {
                jiexieqi.Clear();
                DateTime chaoshi = DateTime.Now;
                int chaoshitime = shebeimodel.DuYanShi;
                ...
                for (; ZongKaiGuan && lianjieqi.TongXinState;)
                {
                    ...
                    //50 00 00 FF FF 03 00 0C 00 10 00 01 04 00 00 01 F5 00 A8 27 00
                    //D0 00 00 FF FF 03 00 50 00 00 00 01 00 01 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00

                    jiexieqi.JieXiWanMeiData((model.Count * 2) + 11, new byte[] { 0xD0, 0x00, 0x00,0xFF, 0xFF,0x03, 0x00 }, false, null, false);
                    if (jiexieqi.DataCount > 0)
                    {
                        byte[] xindata = jiexieqi.GetShiShiData();
                        if (xindata != null && xindata.Length >= model.Count * 2 + 11)
                        {
                            PeiZhiLei.DataMoXing.SetJiCunQiValue(shebeimodel.SheBeiID, xindata.ToList(), model);
                ...
                ChuFaMsg(MsgDengJi.SheBeiBaoWen, $"{shebeimodel.SheBeiName}读取的数据,{(DateTime.Now - chaoshi).TotalMilliseconds}:{ChangYong.ByteOrString(data, " ")}");
                data.Clear();
        ...
        private List<byte> XieZhiLing(int dizhi, int count, byte[] shuju)
        {
            ...
            baowen.Add(0x03);
            baowen.Add(0x00);
            int changdu = 12 + count * 2;
            List<byte> sju2 = CRC.ShiOrByte2(changdu, false).ToList();
            baowen.Add(sju2[0]);
            baowen.Add(sju2[1]);
            ...
            baowen.Add(0x00);
            baowen.Add(0x00);
            List<byte> sju = CRC.ShiOrByte2(dizhi, false).ToList();
            baowen.Add(sju[0]);
            baowen.Add(sju[1]);
            baowen.Add(0x00);
            baowen.Add(0xA8);
