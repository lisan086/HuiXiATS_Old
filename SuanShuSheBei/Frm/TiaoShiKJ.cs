...
namespace YiBanSaoMaQi.Frm
{
    public partial class TiaoShiKJ : UserControl
    {
        ...
        public void SetCanShu(SaoMaModel maModel,PeiZhiLei peiZhiLei)
        {
            ...
            for (int i = 0; i < SaoMaModel.LisJiaoBen.Count; i++)
            {
                FuZhi(SaoMaModel.LisJiaoBen[i]);
            }
        }
        ...
        private void FuZhi(CunModel model)
        {
        ...
        private void dataGridView2_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex>=0)
            {
                if (e.ColumnIndex==4)
                {
                    ...
                    {
                        CunModel model = (this.dataGridView2.Rows[e.RowIndex].Tag as CunModel).FuZhi();
                        model.JiCunQi.Value = ChangYong.TryStr(this.dataGridView2.Rows[e.RowIndex].Cells[2].Value,"");
