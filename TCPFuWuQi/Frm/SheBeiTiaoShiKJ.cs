...
namespace ModBuTCP.Frm
{
    public partial class SheBeiTiaoShiKJ : UserControl
    {
        ...
        public void SetCanShu(SheBeiModel shebeimodel,List<DataCunModel> cunmodel, PeiZhiLei peiZhi)
        {
            PeiZhiLei = peiZhi;
            SheBeiModel = shebeimodel;
            for (int i = 0; i < cunmodel.Count; i++)
            {
                DataCunModel cunModel= cunmodel[i];
                if (cunModel.YingYongType ==YingYongType.DuQingQiuASCII)
                {
                    ZengJiaShuJu(cunModel, 1);
                }
                else if (cunModel.YingYongType ==YingYongType.XieShuJuASCII)
                {
        ...
        private void ZengJiaShuJu(DataCunModel cunModel,int leixing)
        {
            if (leixing == 1)
            {
                ...
                this.dataGridView1.Rows[index].Cells[1].Value = cunModel.SheBeiID;
            ...
            else if (leixing == 2)
            {
                ...
                this.dataGridView2.Rows[index].Cells[1].Value = cunModel.SheBeiID;
