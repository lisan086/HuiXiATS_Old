...
namespace ZhongWangSheBei.ShiXian
{
    public class ZhongShengIO : ABSNSheBei
    {
                    ...
                    if (LisLianJieQi.ContainsKey(cunModel.ZongSheBeiId))
                    {
                        if (LisLianJieQi[cunModel.ZongSheBeiId].TongXinState)
                        {
                            if (cunModel.IsDu.ToString().ToLower().Contains("du") == false)
                            {
                                ...
                                SengData[cunModel.ZongSheBeiId].Add(cunModel);
                            }
        ...
        private void ReadWork(object zhi)
        {
            ...
            int xierutimne = zsmodel.XieRuTime;
            int quehuanshijian = 5;
            if (quehuanshijian <= 0)
            {
                quehuanshijian = 5;
            }
            ...
            while (ZongKaiGuan)
            {
                ...
                try
                {
                    ...
                    if (count>0)
                    {
                        ...
                        for (int i = 0; i < count; i++)
                        {
                            ...
                            if (model!=null)
                            {
                                XieShuJu(zsmodel,model,lianjieqi,xierutimne);
                            }
                ...
                try
                {
                    ...
                    {
                        FuZhi(zsmodel, dingJieXi, lianjieqi);
                    }
        ...
        private void XieShuJu(ZSModel zsmodel,CunModel cunmodel, ABSLianJieQi lianjieqi, int xierutime)
        {
            PeiZhiLei.DataMoXing.SetSate(cunmodel,0);
            bool zhen = false;
            List<List<byte>> zhiling = PeiZhiLei.DataMoXing.GetCMD(cunmodel);
            if (zhiling.Count > 0)
            {
        ...
        private void FuZhi( ZSModel model, SerialDataGuDingJieXi serialDataGuDingJieXi,ABSLianJieQi lianjieqi)
        {         
            if (model.ZhiLing != null)
            {
                List<int> dizhis = model.ZhiLing.Keys.ToList();
                for (int c = 0; c < dizhis.Count; c++)
                {
                    int dizhi=dizhis[c];
                    ResultModel rm = lianjieqi.Send(model.ZhiLing[dizhi].ToArray());
                    ChuFaMsg(MsgDengJi.SheBeiBaoWen, rm.Msg);
                    ...
                    for (; ZongKaiGuan && lianjieqi.TongXinState;)
                    {
                        ...
                        serialDataGuDingJieXi.JieXiWanMeiData(model.ChangDu + 5, new byte[] { (byte)dizhi, 0x03, (byte)(model.ChangDu) }, true, JiaoYanWenDu, false);
                        if (serialDataGuDingJieXi.DataCount > 0)
                        {
                            ...
                            if (xindata != null && xindata.Length >= model.ChangDu + 3)
                            {
                                ...
                                PeiZhiLei.DataMoXing.SetJiCunQiDuValue(model.SheBeiID, dizhi, shujus);
                                model.TxCiShu[dizhi] = 5;
                            }
                            break;
                    ...
                    if (model.TxCiShu[dizhi] < 0)
                    {
                        PeiZhiLei.DataMoXing.SetTX(model.SheBeiID, dizhi, false);
                    }
                    ChuFaMsg(MsgDengJi.SheBeiBaoWen, $"{model.SheBeiID}读取的数据,{(DateTime.Now - date).TotalMilliseconds}:{ChangYong.ByteOrString(datas, " ")}");
                    datas.Clear();
                    if (dizhis.Count>1)
                    {
        ...
        public override TxModel GetMeiGeTx()
        {
            ...
            {
                List<int> dizhis = PeiZhiLei.DataMoXing.LisSheBei[i].ZhiLing.Keys.ToList();
                foreach (var item in dizhis)
                {
                    ZiTxModel zmodel = new ZiTxModel();
                    zmodel.Tx = PeiZhiLei.DataMoXing.LisSheBei[i].Tx[item];
                    zmodel.ZiSheBeiID = PeiZhiLei.DataMoXing.LisSheBei[i].SheBeiID;
                    zmodel.ZiSheBeiName = $"{PeiZhiLei.DataMoXing.LisSheBei[i].Name}-{item}"; 
                    if (zmodel.Tx == false)
                    {
                        ischengg = false;
                    }
